...
// ── Historical data point (FRED series element) ───────────────────────────────
// v1 regression fix: MacroSnapshot stores full series arrays, not flat numbers.
// getLatestValues() derives Record<string, number> from these for agent prompts.
...
// Full historical series — stored in macro_snapshot.json
// Enables trend computation: 3m_change, nfp_3m_avg, oil_price_3m_change, etc.
export const MacroSnapshotSchema = z.object({
  series:    z.record(z.string(), z.array(DataPointSchema)),
  fetchedAt: z.record(z.string(), z.string()),  // per-series ISO timestamp
});

// Derived flat snapshot — passed to agent prompts
// Computed from MacroSnapshotSchema by fredFetcher.getLatestValues()
// Includes both raw latest values AND computed trend indicators
export type MacroFlatSnapshot = Record<string, number>;
// Example keys: { cpi_yoy: 3.2, yield_30y: 4.73,
//                oil_price_3m_change: 0.12, nfp_3m_avg: 145000,
//                real_wages: -0.4,  yield_curve_30_2: 0.85 }
...
export const PositionConfigSchema = z.object({
  ...
  // v1 regression fix: stop and targets are optional —
  // not all positions have pre-defined stops (e.g. SGOV cash buffer, IBKR grant-restricted).
  // eodMonitor skips stop check silently if stop is absent.
  ...
  deadline:             z.string().optional(),   // ISO date string; speculative only
...
export const RebalancingOutputSchema = z.object({
  ...
  fastest_path_to_being_wrong:      z.string(),
  // v1 regression fix: evaluatedAt restored — required for stale-data guard
  // and for correlating rebalancing output with regime snapshot in decision log
  evaluated_at:                     z.string(),   // ISO datetime string
});
...
**Public interface:**
```typescript
// Full historical series — up to `limit` most recent observations
async function fetchSeries(seriesId: string, limit: number = 24): Promise<DataPoint[]>

// Fetches all FRED_SERIES, merges with manual indicators from manualIndicators.ts
// Returns MacroSnapshot written to src/data/cache/macro_snapshot.json
async function fetchAll(): Promise<MacroSnapshot>

// Derives MacroFlatSnapshot from a MacroSnapshot for use in agent prompts
// Returns: raw latest values + computed trend indicators (see below)
function getLatestValues(snapshot: MacroSnapshot): MacroFlatSnapshot
```
...
These computed fields are stored in `macro_snapshot.json` under a `"derived"` key
alongside raw series data so they are cacheable between calls, not recomputed each time.

**`macro_snapshot.json` structure:**
```json
{
  "series": {
    "cpi_yoy":   [{ "date": "2026-04-01", "value": 3.4 }, ...],
    "yield_30y": [{ "date": "2026-05-14", "value": 4.73 }, ...]
  },
  "derived": {
    "nfp_3m_avg":          145000,
    "oil_price_3m_change":  0.12,
    "real_wages":          -0.4,
    "yield_curve_30_2":     0.85,
    "credit_spread_delta":  0.22
  },
  "fetchedAt": {
    "cpi_yoy":   "2026-05-15T07:00:00Z",
    "yield_30y": "2026-05-15T07:00:00Z"
  }
}
```

**Caching:** Full `MacroSnapshot` written to `src/data/cache/macro_snapshot.json`.
Stale thresholds: 24h for daily series, 7 days for quarterly (GDP).
`getLatestValues()` checks per-series `fetchedAt` before returning — rejects
stale series individually rather than invalidating the entire snapshot.
...
interface RebalancingAgentInput {
  ...
  macroSnapshot:     MacroFlatSnapshot;
}
